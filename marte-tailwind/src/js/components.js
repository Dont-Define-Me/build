...
(function() {
  function initAlertCard(card) {
    card.addEventListener('click', function(event) {
      if(event.target.closest('.js-alert-card__close-btn')) Util.addClass(card, 'hidden');
    });
  };
...
(function() {
  ...
  function hoverChart(chart, event) {
    ...
    if(selectedIndex !== false && selectedIndex !== chart.selectedIndex) {
      ...
      placeTooltip(chart);
      Util.removeClass(chart.tooltip[0], 'hidden');
    }
  ...
  function resetTooltip(chart) {
    ...
    Util.addClass(chart.tooltip[0], 'hidden');
    chart.hovering = false;
...
(function() {
  ...
  function setXAxis(chart) {
    ...
    for(var i = 0; i < xLabels.length; i++) {
      ...
      var labelClasses = (chart.options.xAxis && chart.options.xAxis.labels) ? 'chart__axis-label chart__axis-label--x js-chart__axis-label' : 'hidden js-chart__axis-label';
  ...
  function setYAxis(chart) {
    ...
    for(var i = yLabels.length - 1; i >= 0; i--) {
      var textEl = document.createElementNS('http://www.w3.org/2000/svg', 'text');
      var labelClasses = (chart.options.yAxis && chart.options.yAxis.labels) ? 'chart__axis-label chart__axis-label--y js-chart__axis-label' : 'hidden js-chart__axis-label';
      Util.setAttributes(textEl, {class: labelClasses, 'alignment-baseline': 'middle'});
  ...
  function createDataTable(chart, index) {
    ...
    table = (chart.options.xAxis && chart.options.xAxis.legend) 
      ? table + '<th scope="col-span-12">'+chart.options.xAxis.legend+'</th>'
      : table + '<th scope="col-span-12"></th>';
      
    table = (chart.options.yAxis && chart.options.yAxis.legend) 
      ? table + '<th scope="col-span-12">'+chart.options.yAxis.legend+'</th>'
      : table + '<th scope="col-span-12"></th>';
  ...
  function initTooltips(chart) {
    ...
    if(chart.options.yIndicator) {
      ...
      Util.setAttributes(tooltipLine, {x1: 0, y1: chart.topDelta, x2: 0, y2: chart.topDelta + chart.yAxisHeight, transform: 'translate('+chart.leftDelta+' '+chart.topDelta+')', class: 'chart__y-indicator js-chart__y-indicator hidden'});
    ...
    if(chart.tooltipOn) {
      ...
      tooltip.setAttribute('class', 'chart__tooltip js-chart__tooltip hidden '+chart.tooltipClasses);
  ...
  function resetTooltip(chart) {
    ...
    if(chart.tooltipOn) Util.addClass(chart.tooltip, 'hidden');
    if(chart.options.yIndicator)Util.addClass(chart.interLine, 'hidden');
    resetMarkers(chart, false);
    resetBars(chart, false);
...
(function() {
  ...
  function resetTriggerLabel(datePicker, bool) {
    ...
    if(bool) {
      Util.addClass(datePicker.triggerLabelWrapper[0].children[0], 'hidden');
      Util.removeClass(datePicker.triggerLabelWrapper[0].children[1], 'hidden');
      // place calendar
      placeCalendar(datePicker);
    } else if( !datePicker.dateSelected[0] && !datePicker.dateSelected[1]) {
      Util.addClass(datePicker.triggerLabelWrapper[0].children[1], 'hidden');
      Util.removeClass(datePicker.triggerLabelWrapper[0].children[0], 'hidden');
    }
  };
  ...
  function initPredefinedOptions(datePicker) {
    ...
    if(select.options[select.selectedIndex].value == 'custom') Util.removeClass(datePicker.element, 'hidden');

    select.addEventListener('change', function(event) {
      if(select.options[select.selectedIndex].value == 'custom') {
        // reveal date picker
        Util.removeClass(datePicker.element, 'hidden');
        placeCalendar(datePicker);
        datePicker.trigger.focus();
      } else {
        Util.addClass(datePicker.element, 'hidden');
      }
    });
  };
    ...
    new Chart({
      ...
      tooltip: {
        ...
        customHTML: function(index, chartOptions, datasetIndex) {
          return '<span class="text-contrast-medium">'+chartOptions.xAxis.labels[index] + ':</span> $'+chartOptions.datasets[datasetIndex].data[index]+'';
        }
      },
      animate: true
    });
  ...
  if(areaChart3) {
    new Chart({
      ...
      tooltip: {
        ...
        position: 'top',
        customHTML: function(index, chartOptions, datasetIndex) {
          var html = '<p class="mb-1.5">Total '+chartOptions.xAxis.labels[index] + '</p>';
          html = html + '<p class="flex items-center"><span class="h-2 w-2 rounded-full bg-primary mr-1.5"></span>$'+chartOptions.datasets[0].data[index]+'</p>';
          html = html + '<p class="flex items-center"><span class="h-2 w-2 rounded-full bg-accent mr-1.5"></span>$'+chartOptions.datasets[1].data[index]+'</p>';
          return html;
        }
      },
      animate: true
    ...
    new Chart({
      ...
      tooltip: {
        ...
        customHTML: function(index, chartOptions, datasetIndex) {
          var html = '<p class="mb-1.5">Total '+chartOptions.xAxis.labels[index] + '</p>';
          html = html + '<p class="flex items-center"><span class="h-2 w-2 rounded-full bg-primary mr-1.5"></span>$'+chartOptions.datasets[0].data[index]+'</p>';
          html = html + '<p class="flex items-center"><span class="h-2 w-2 rounded-full bg-contrast-higher mr-1.5"></span>$'+chartOptions.datasets[1].data[index]+'</p>';
          return html;
        },
        position: 'top'
      },
  ...
  if(lineChart2) {
    ...
    new Chart({
      ...
      tooltip: {
        enabled: true,
        customHTML: function(index, chartOptions, datasetIndex) {
          return '<span class="text-contrast-medium">'+getNiceDate(chartOptions.datasets[datasetIndex].data[index][0])+' - </span> '+chartOptions.datasets[datasetIndex].data[index][1] + '°C';
        }
      },
      animate: true
    });
...
(function() {
  ...
  function toggleActions(table, selection) {
    if(table.actionsSelection && table.actionsSelection.length > 0) {
      Util.toggleClass(table.actionsSelection[0], 'hidden', !selection);
    }
    if(table.actionsNoSelection && table.actionsNoSelection.length > 0) {
      Util.toggleClass(table.actionsNoSelection[0], 'hidden', selection);
    }
  };
